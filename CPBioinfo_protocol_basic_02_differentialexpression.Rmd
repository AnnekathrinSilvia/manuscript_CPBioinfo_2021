...
ideal guides the user through different aspects of a DE analysis, starting from the data upload, through the `DESeq2` [@Love2014] workflow including normalization, filtering, exploration of differentially expressed genes, to functional analysis and gene signature exploration.
...
*Files* 
  ...
  The files are expected to be tab-separated, (**TODO but also comma- or semicolon-separated files are accepted?**) (see **Alternative protocol 1**). 
...
The second option is to provide the data as a `DESeqDataSet` object or as a `DESeqResult object`, e.g. `ideal(dds)` where `dds` is the `DESeqDataSet` object [@Love2014]. 
...
When launching the application, the landing page of ideal is the "Welcome" panel providing the user information about the application and how to use it. 
A control sidebar is located on the left side of the user interface. 
It contains several input controls which affect different tab panels. By changing one or more of the input parameters, the user can get a fine control on what is computed and displayed. 
Moreover, the sidebar displays a list of the underlying objects with which basically all of the analyses can be performed. 
...
After the `DESeqDataSet` object was generated, the ValueBox on top turns green, and also the sidebar field for it will get a green check mark [@Love2014]. The two light blue boxes that have appeared provide the optional steps for adding the appropriate annotation and discarding unwanted samples. 
...
![**Figure ** Data Upload panel - **TODO**.]([**TODO**])

2. Inspect and filter the count matrix. 
For this navigate to the panel "Counts Overview", which provides an interactive table, which can display raw, normalized, and log-normalized values for all the genes and samples in the data. 
...
![**Figure ** Counts Overview panel - **TODO**.]([**TODO**])

3. Navigate to the "Extract results" panel to calculate the DE. 
...
Further filtering options are provided to the user, where independent filtering or the Independent Hypothesis Weighting (IHW) [**TODO**] can be defined. 
...
The interactive table directly links the gene symbols and the Ensembl identifiers to external databases for that feature, either [ensemble.org](https://www.ensembl.org/index.html) or the [NCBI Gene DB](https://www.ncbi.nlm.nih.gov/gene). 
![**Figure ** Extract Results panel - **TODO**.]([**TODO**])
...
5. Navigate to the "Summary Plots" panel. 
...
This will display a box plot for the expression values in all conditions, and additional info retrieved from the Entrez database. 
Below the gene expression box plot, explore the volcano plot, where the significance is directly plotted against the effect size. 
...
The selected genes can be further explored in the collapsible element at the bottom of the tab panel.

![**Figure ** Summary Plots panel - **TODO**.]([**TODO**])

![**Figure ** Summary Plots panel - **TODO**.]([**TODO**])

6. Navigate to the panel “Gene Finder” and explore a subset of genes of interest. 
In this protocol we will define the genes `CCL5`, `IFNGR1`, and `CXCL11` as genes of interest in the sidebar.
The tab panel displays up to four gene expression box plots for the selected genes, and also highlights the genes in the MA plot. 
Below the MA plot the user finds a downloadable and interactive table containing various relevant DE parameters to all differentially expressed genes.
The user can search for features of interest or sort the table according to single columns. 
To avoid selecting many genes by hand (from the selectize widget in the sidebar), one can also upload a list as a plain text file containing one column of features with one feature per row.
The genes from the uploaded file are highlighted in the MA plot below. 
Again, a downloadable, and interactive table visualizing the DE data of the uploaded genes appears below the last MA plot.

![**Figure ** Gene Finder panel - **TODO**.]([**TODO**])

7. Navigate to the "Functional Analysis" tab to perform a gene set enrichment analysis on the differentially expressed genes and to gain insight on the affected biological pathways. 
Make sure, `GO Biological Process` is set in the select widget called "Select the GO category(ies) of interest". 
Here, the user can select from the three GO categories 1. `GO Biological Process`, 2. `GO Molecular Function`, and 3. `GO Cellular Component`.
The gene set enrichment analysis can be performed either only on upregulated (tab panel "UPregu"), on downregulated (tab panel "DOWNregu"), or on both up- and downregulated genes (tab panel "UPDOWN"). 
Additionally, up to two custom gene lists can be uploaded in the respective tab panels "List1" and "List2".
Navigate to the "UPDOWN" tab panel to perform the gene set enrichment analysis on the up- and downregulated features.
Three methods of overrepresentation analysis (ORA) are available: 1. The standard ORA based on the `limma` package [PMC4402510], 2. the `goseq` ORA, which accounts for the specific length bias intrinsic in RNA-Seq assays (longer genes have higher chances of being called DE), and 3. the `topGO` ORA, which decorrelates the Gene Ontology graph structure and is particularly valuable for pruning terms which are topologically less meaningful than their specific nodes.
...
An interactive, and downloadable table appears listing the relevant GO terms and further data, e.g. the list of differentially expressed genes respective to the GO term.
Click on the blue button of the GO term `GO:0060333` to get further information from the [AmiGO](http://amigo.geneontology.org/amigo) database. As shown in the "Term" column of the interactive table, this GO term is associated with the interferon-gamma-mediated signaling pathway.
Click on the row of the GO term `GO:0060333` to inspect the appearing heatmap.
The heatmap displays all the  all the differentially expressed genes annotated to the particular term colored by the normalized transformed values.
This allows to compare the gene expression between the samples.
Further down the page, explore the intersection between the uploaded gene lists and the regulated gene set.
Tag the options `Use up regulated genes`, `Use down regulated genes`, and remove the checks from the remaining options to get a Venn diagram of up- and downregulated features, as well as the appealing alternative from the `UpSetR` package [PMC5870712], where all combination of sets are explicitly shown.

![**Figure ** Functional Analysis panel - **TODO**.]([**TODO**])

8. Navigate to the "Signature Explorer" panel.
Upload the `h.all.v7.0symbols.gmt` file retrieved from the [MSigDB](http://software.broadinstitute.org/gsea/downloads.jsp) (please refer to the **Alternative protocol 1** for further detail on downloading the file). 
The central green info box will report the number of signatures contained - in this case 50 signatures are present in the uploaded GMT file.
Compute the variance stabilized transformed data by clicking of the equally named button.
To match the identifier types of your data with the signatures, select `ENSEMBL` as the ID type in your dds data, `SYMBOL` as the ID type of your signatures, and `org.Hs.eg.db` as the organism package for matching.
Click the button "Apply id conversion between data and signatures" to compute the matching.
Further, select the gene signature of interest, e.g. `HALLMARK_INTERFERON_GAMMA_RESPONSE` and `condition` in the selection widget called "Select the colData to decorate". Optionally, use the check box below to use only DE genes of the signature.
A heatmap appears at the bottom of the panel, which can be customized by using further check boxes, e.g. "Center mean", "Cluster rows", and "Cluster columns".

![**Figure ** Signature Explorer panel - **TODO**.]([**TODO**])

9. Navigate to the panel "Report Editor" to generate and download a report of the analysis containing the current state of the parameters and generated images.
Check `HTML` in the info box "markdown options" to get an HTML file as output.
Provide a Title and the Authors name.
Additionally, customize the style of the final document by defining the selections "Table of Content", "Number sections", and selecting a theme e.g. `Cerulean`.
Preview the report below in the "Report preview" tab panel.
Experienced users can edit the R Markdown of the report in the "Edit report" tab panel.
Click on "Generate & Save" to save the report on your system.
Click on "Export as serialized SummarizedExperiment" to download the R data object as a `SummarizedExperiment` object for further use.

![**Figure ** Report Editor - **TODO**.]([**TODO**])
