...
In the yellow "Step 2" box you can select the experimental design; specify `line` and `condition` for the `macrophage` dataset, and click on the green action button ("Generate the dds object"). 
This specification will enable you to estimate the effect size of the "condition" variable, while controlling for the "cell" line of origin, leveraging the flexible generalized linear model framework.
...
IRF1, IL18BP, and GBP2 are listed in the table below as the top regulated genes, sorted by adjusted p-value. 
...
**Explore overviews of the results and single genes of interest**

10. Generate overview visual representation of the DE results.
Navigate to the "Summary Plots" panel to generate some visual summaries of the results. 
...
A zoomed-in version of the MA plot focused on the selected area will appear on the right side, providing gene symbols if these are defined in the annotation object. 
...
The information on the selected genes can be further exported from the collapsible element at the bottom of the tab panel.
...
The table containing all relevant parameters for the genes included in the selector widget can be explored in the collapsible element, and downloaded as text file.
...
**Perform functional enrichment analyses on the data and explore gene signatures.**
...
Navigate to the "Signature Explorer" panel.
Upload the `h.all.v7.0symbols.gmt` file retrieved from the [MSigDB database](http://software.broadinstitute.org/gsea/downloads.jsp) (please refer to the **Alternative protocol 1** for further detail on downloading the file). 
