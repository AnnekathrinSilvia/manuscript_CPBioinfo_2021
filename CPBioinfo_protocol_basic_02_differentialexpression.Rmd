...
The tab panel displays up to four gene expression boxplots for the selected genes, and also highlights the genes in the MA plot. 
...
Make sure, `GO Biological Proces` is set in the select widget called "Select the GO category(ies) of interest". 
Here, the user can select from the three GO categories 1. `GO Biological Proces`, 2. `GO Molecular Function`, and 3. `GO Cellular Component`.
...
Tag the options `Use up regulated genes`, `Use down regulated genes`, and remove the checks from the remaining options to get a Venn diagramm of up- and downregulated features, as well as the appealing alternative from the `UpSetR` package [PMC5870712], where all combination of sets are explicitly shown.
...
Compute the variance stabilized transformated data by clicking of the equally named button.
...
Further, select the gene signature of interest, e.g. `HALLMARK_INTERFERON_GAMMA_RESPONSE` and `condition` in the selction widged called "Select the colData to decorate". Optionally, use the check box below to use only DE genes of the signature.
...
Click on "Export as serialized SummerizedExperiment" to download the R data object as a `SummerizedExperiment`object for further use.
