...
[TODO: this last paras, maybe to be moved to Support Protocol instead of repeating?]
...
*Files* 
  ...
  - `res_enrich`, a `data.frame` with the results of functional enrichment analysis (see section "Generate the required input for running GeneTonic" below for details on the expected file formatting);
...
GeneTonic's usage is envisioned once the usual set of results from a differential expression analysis workflow have been computed [FIGURE_HERE]. 
...
We refer to the original publication of GeneTonic for more details on how these scores are computed [@Marini2021].
...
**Running the GeneTonic application.**
...
This can be exactly a GeneTonicList containing all the information regarding the experiment and setting of interest, as exemplified in the chunk below - you can also load a precomputed object, provided in the `manuscript_CPBioinfo_2021` repository.
...
GeneTonic's layout [FIGURE_HERE] is built on the Bootstrap 4 components, provided via the `bs4Dash` package.
This includes: 
* A header bar, where dropdown menus link to further documentation, and where the bookmark button is located - useful for recording features of interest during an interactive session [FIGURE_HERE]
* A sidebar on the left, as the main way to access the different functionalities provided in the app [FIGURE_HERE]
* A control bar on the right, where users can find most widgets to customize the appearance of the output (plots, tables, graphs) [FIGURE_HERE]
* The main body of the application, where different tabs are activated by clicking on the respective icons or text in the sidebar. [FIGURE_HERE]
  In each of the panels, a context-specific interactive tour can be started by clicking on its respective button in the top right corner. [FIGURE_HERE]
...
Open the collapsible elements, "Expression Matrix", "DE results", "Functional analysis results", and "Annotation info", by clicking on the plus sign [FIGURE_HERE].
Each of them presents a tabular view of the elements of the provided GeneTonicList.
The value boxes below give a rapid overview on the dimensions of these objects [FIGURE_HERE].
...
Navigate the GeneTonic app to the Gene-Geneset tab panel.
...
By default, 15 genesets are displayed; this behavior can be changed by setting a different value in the control bar (right side of the app) [FIGURE_HERE].
The bipartite graph underlying the Gene-Geneset network contains two sets of entities, and their connections reflect the membership of differentially expressed genes to the subset of genesets selected [FIGURE_HERE].
This network can be explored in an interactive manner, avoiding the static "hairball" effect that would derive if a large number of genesets is selected, or if many nodes are present (irrespective of their type).
It is possible to zoom in the network by scrolling, pan by clicking and holding the left mouse button, select by hovering or clicking on a node of interest [FIGURE_HERE].
Additionally, the "Select by id" dropdown selector allows user to search for a specific node, while presenting them as an alphanumerically sorted list [FIGURE_HERE].
...
Users can move around the nodes by dragging and dropping them, and eventually save a static snapshot of this as a png image (by clicking on the dedicated "Save ggs graph" button) [FIGURE_HERE].

12. Explore in depth the nodes of the network.
Click on any geneset in the network [FIGURE_HERE].
...
Extra information, as retrieved from the GO.db annotation package, is displayed below, with a link to reach the corresponding entry in the AmiGO database [FIGURE_HERE].
On the other hand, genes are marked as oval-shaped nodes, with a color scale mirroring the expression change in the differential expression comparison [FIGURE_HERE].
When clicking on a gene, the normalized expression values are displayed, grouped by the experimental covariates selected [FIGURE_HERE].
...
If users intend to reproduce the output of the network in a later session, detached from the execution as a web application, the button with a user-edit icon opens up a modal dialog window, where the code to recreate the content is automatically generated and provided [FIGURE_HERE]; this also serves as a bridge to learn how to assemble all the individual analysis components into a full scripted report.

13. Bookmark a subset of genes and genesets.
While exploring the network, if a node is selected, it is possible to bookmark it by either clicking on the Bookmark button in the header, or by using the left Ctrl key as a shortcut [FIGURE_HERE].
Repeat this operation until a number of nodes has been selected, making sure to select entities from both categories.
The bookmarks are stored for the duration of the session, and can be previewed in the Bookmarks tab panel [FIGURE_HERE].

14. Calculate and explore the backbone of the Gene-Geneset network.
Below the main network, users can find the functionality to explore a backbone of this bipartite graph [FIGURE_HERE].
...
As a complement to this, the table on the right side ranks the genes according to their degree of connectivity; this can be important to identify hub-like genes that might be involved in a multitude of biological processes [FIGURE_HERE].
Similar to the Gene info box, links to external databases are automatically provided, to simplify the further steps of exploration when trying to generate new hypotheses by cross-referencing the own project with published literature.

  
**Building an interactive enrichment map for the functional enrichment results.**

15. Create an interactive enrichment map.
Navigate the GeneTonic app to the Enrichment Map tab panel; again, use the left sidebar for this purpose.
The setting you used in the previous steps is kept, so make sure to adjust it to a meaningful value - in this case, you can select 75 genesets to be simultaneously displayed [FIGURE_HERE].
...
By default, the enrichment map colors the node by the geneset p-value resulting from the enrichment test, but this can be changed by selecting from another entry from the dropdown menu in the top left corner of the main content [FIGURE_HERE].
For example, coloring by Z score is useful to obtain an overall sense of how the pathway activities are changing (see how the immune response cluster is being mostly activated, while the DNA replication is negatively regulated [FIGURE_HERE]).
...
For example, select from the ids listed in the dropdown the geneset "response to interferon-gamma" (GO:0034341); the sample groups are very well separating in all the 4 subgroups included in the expression dataset [FIGURE_HERE].
...
The "Geneset distillery", activated in the collapsible element in the lower half of the page, enables the detection of cluster by applying the Markov clustering algorithm (selected by default) on the graph object returned as enrichment map [FIGURE_HERE].
...
The tabular output reports for each cluster a summary of its components, listing as representative term the pathway with the highest significance [FIGURE_HERE].
This is complemented by the meta-geneset heatmap, which can provide a more granular view on the union set of genes assigned to at least any of these genesets [FIGURE_HERE].
...
The geneset volcano plot displays the significance of each pathway against the Z score (as an indicator of the direction of expression change [FIGURE_HERE]).
...
The enhanced table displays for each geneset the contribution (in log2 fold change from the DE analysis) of the individual members [FIGURE_HERE]; an interactive version, based on the plotly framework, can additionally show details as tooltip, reducing the issue when many genes are presented.
...
A diverse spectrum of visualization techniques has been adopted for describing at a higher level the functional enrichment results.
Navigate to the "GSViz" panel in the GeneTonic app [FIGURE_HERE].
This includes e.g. a scores heatmap, where the geneset activity gets summarized at the sample level, and presented in a heatmap - the other plots can be shown by navigating through the different tabs.
...
Each of these plots can be reproduced with the code displayed by pressing on the button below the graphics [FIGURE_HERE].
...
The two tables in the main body are displaying the respective list of genes and of genesets users have been selecting in the interactive session [FIGURE_HERE], irrespective from which panel they were shortlisted.
...
Clicking on the dedicated button [FIGURE_HERE] writes a serialized rds file, that can later be imported in an R session, and provided as input to the iSEE main function [@Rue-Albrecht2018].
...
Click the "Start the happy hour" button to initiate the compilation and generation of a RMarkdown report, able to pick the current status of the reactive elements of the app [FIGURE_HERE].
A comprehensive default template Rmd report is provided within the GeneTonic package, and expert users can additionally extend and edit that at will.
...
Notably, the reporting functionality is available also in batch mode, to benefit from the streamlined aspects of a scripted analysis [ref to alt protocol here? there we can run this from the R REPL].
