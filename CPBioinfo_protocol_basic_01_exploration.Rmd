...
The main pcaExplorer function requires the count matrix, metadata, and annotation to be either loaded into the environment before the main call, or offers the possibility to load these objects during runtime (which can be common for a first run).
If loading the input files at runtime, the application can be launched with the simple command:

```{r, eval=FALSE}
pcaExplorer()
```

leaving all parameters unspecified.
...
where `countmatrix`, `metadata` and `annotation` have to be substituted by the names of the respective objects.
...
Ensure that both options 'Use log2 values for plot axes and values' and 'Use a subset of max 1000 genes (quicker to plot)' are selected, to reduce the computation and rendering time. 
...
Here you will see the number of uniquely aligned reads per sample, accompanied by some information about the expressed genes in the data. 
...
To color the samples by condition, click on the 'Group/color by' field and select 'condition' from the drop-down menu - the PCA plot should now display the samples in 4 colors (Fig. X). 
...
Navigate to the options panel on the far left and search for the 'Nr. of (most variant) genes' option. 
The default number is 300 genes. 
Change the number of genes to 500. 
To change the number of genes either use the increase and decrease buttons in the option field (indicated by small arrows) or type the number in the box; if using the second option, make sure to delete the previously selected number first (by hitting the delete key on your keyboard until the number disappeared). 
Inspect the PCA plot and compare it with the plot in Fig. X. 
The values for the percent of variance explained of the first and second principal component should have changed. 
If you increase or decrease the number of selected genes further, you should also be able to observe a slight difference in the clustering of the samples.
...
This can be extremely relevant when analyzing large datasets where it is hard to identify a clean structure like the one illustrated in this Basic Protocol.

12. Zoom in the PCA plot.
The Sample View panel provides the possibility to zoom in on the PCA. 
For this, the Shiny brushing mechanism is used: hover your mouse over the PCA plot until your mouse cursor changes to a little plus sign. 
...
13. Identify genes with high loadings on the main principal components.
...
18. Inspect any gene of interest in the GeneFinder.
...
Select the option 'condition' on the left in the 'Group/color by' selector.
...
19. Compute a functional interpretation for the principal components.
